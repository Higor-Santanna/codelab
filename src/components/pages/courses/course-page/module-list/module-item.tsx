...
export const ModuleItem = ({ data }: ModulesItemProps) => {
    const totalLessons = data.lessons.length;
    const totalDuration = data.lessons.reduce(
        (acc, lesson) => acc + lesson.durationInMs, 0
    );
    ...
    return (
        <Accordion.Item value={data.id}
            ...
            <Accordion.Trigger className="flex items-center gap-4 p-4 w-full hover:bg-muted/50 transition-all outline-none">
                <div className={cn(
                    "w-10 h-10 min-w-10 rounded-full flex items-center justify-center font-semibold bg-black/70 relative transition-all", moduleProgress >= 100 && "text-primary bg-primary/10"
                )}>
                    {data.order}
                    <CircularProgress progress={moduleProgress}
